...
(function (factory) {
  ...
  var range = (function () {
    ...
    var WrappedRange = function (sc, so, ec, eo) {
      ...
      /// CHANGE BY MH
      /////////////////////////////
      this.selectFromTo = function(from, to){
      ...
      this.expandWord = function(){
        var boundaryPoints = this.getPoints();
        if(boundaryPoints.sc.nodeType == 3){
          var s = boundaryPoints.sc.nodeValue;
          if(boundaryPoints.so < s.length){ // not already at the end
            var specialChars = [" "," ",",",";","(",")","[","]","{","}",".","!","?","=","+","*","-","/"];
            while(boundaryPoints.so > 0 && 
                  specialChars.indexOf(s[boundaryPoints.so]) == -1) boundaryPoints.so--;
            while(boundaryPoints.eo < s.length && 
                  specialChars.indexOf(s[boundaryPoints.eo]) == -1) boundaryPoints.eo++;

            if(boundaryPoints.so > 0 && boundaryPoints.so < s.length) boundaryPoints.so++;
            if(boundaryPoints.eo > boundaryPoints.so && boundaryPoints.eo > 0) boundaryPoints.eo--;
            
            if(specialChars.indexOf(s[boundaryPoints.eo]) == -1)
             boundaryPoints.eo = Math.min(boundaryPoints.eo+1,s.length); // not part of word      
...
//        ['#630000', '#7B3900', '#846300', '#295218', '#083139', '#003163', '#21104A', '#4A1031']
  ...
  var Bullet = function () {
    ...
    // MH: a) b) c) list style
    this.insertOrderedListAbc = function () {
      this.toggleList('OLabc');
    };
    /**
    ...
    this.wrapList = function (paras, listName) {
      ...
      listNode.style.listStyleType = listStyle; 
  ...
  var Editor = function () {
     ...
     * @param {jQuery} $editable
     */
     // MH: insert a) b) c) list
    this.insertOrderedListAbc = function ($editable) {
      bullet.insertOrderedListAbc($editable);
      afterCommand($editable);
    };
  ...
  var History = function ($editable) {
    ...
    this.redo = function () {
      if (stack.length - 1 > stackOffset) {
        ...
        /// CHANGE BY MH
        /////////////////////////////
        // delete last stack because of first undo
        if(stackOffset == stack.length -1){
         stackOffset--;
         stack.pop();
        }
  ...
  var Button = function () {
    ...
    this.update = function ($container, styleInfo) {
      ...
      btnState('button[data-event="insertOrderedList"]', function () {
        return styleInfo['list-style'] === 'ordered' && styleInfo['list-style-type'] != 'lower-alpha';
      });
      btnState('button[data-event="insertOrderedListAbc"]', function () {
        return styleInfo['list-style'] === 'ordered' && styleInfo['list-style-type'] === 'lower-alpha';
      });
  ...
  var Renderer = function () {
    ...
    var tplButtonInfo = {
        ...
        var dropdown = '<ul class="dropdown-menu">' +
                         '<li>' +
                           '<div class="btn-group">' +
                             ...
                             '<div class="note-color-reset btn btn-default btn-xs" data-event="backColor"' +
                           ...
                           '<div class="btn-group">' +
                             ...
                             '<div class="note-color-reset btn btn-default btn-xs" data-event="foreColor" data-value="inherit" title="' + lang.color.reset + '">' +
      ...
      olAbc: function (lang) {
        return tplIconButton('fa fa-list-ol', {
          event: 'insertOrderedListAbc',
          title: lang.lists.ordered+ " abc"
        });
      },
      // MH: List drop down
      lists: function (lang) {
        var ulButton = tplIconButton('fa fa-list-ul', {
          title: lang.lists.unordered,
          event: 'insertUnorderedList'
        });
        var olButton = tplIconButton('fa fa-list-ol', {
          title: lang.lists.ordered,
          event: 'insertOrderedList'
        });
        var olAbcButton = tplIconButton('fa fa-list-ol', {
          title: lang.lists.ordered+ " abc",
          event: 'insertOrderedListAbc'
        });
        ...
        var dropdownList = '<div class="dropdown-menu">' +
                         '<div class="note-list btn-group">' +
                           ulButton + olButton + olAbcButton +
                         '</div>' +
                         '<div class="note-list btn-group">' +
      ...
      $container.find('.note-color-palette').each(function () {
        ...
        for (var row = 0, lenRow = colorInfo.length; row < lenRow; row++) {
          ...
          for (var col = 0, lenCol = colors.length; col < lenCol; col++) {
            var c = colors[col];
            var ce = $('<div class="color_'+c+'"></div>');
            $("body").append(ce);
            var color = ce.css("color");
            ce.remove();
