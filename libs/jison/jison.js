...
Jison.print = function print () {console.log(arguments);};
...
generator.constructor = function Jison_Generator (grammar, opt) {
    ...
    if (grammar.lex) {
        // change MH, add unused terminals from lexer too
        if(grammar.lex.tokennames){
          for(var i=0; i < grammar.lex.tokennames.length; i++){
            if(this.symbols.indexOf(grammar.lex.tokennames[i]) == -1){
              var id = this.symbols.length;
              this.symbols.push(grammar.lex.tokennames[i]);
              this.terminals.push(grammar.lex.tokennames[i]);
              this.terminals_[id] = grammar.lex.tokennames[i];
              this.symbols_[grammar.lex.tokennames[i]] = id;
            }
          }
        }
...
generator.processGrammar = function processGrammarDef (grammar) {
    ...
    if (tokens) {
        if (typeof tokens === 'string') {
            // CHANGE MH: Support Whitespace
            var trimed = (tokens.replace(/\u00A0/g,"łłł").trim()).replace(/łłł/g,"\u00A0");
            tokens = trimed.split(' ');
        } else {
        ...
        prods.forEach(function buildProds_forEach (handle) {
            ...
            if (handle.constructor === Array) {
                if (typeof handle[0] === 'string'){
                    // CHANGE MH: Support Whitespace
                    var trimed = (handle[0].replace(/\u00A0/g,"łłł").trim()).replace(/łłł/g,"\u00A0");
                    rhs = trimed.split(' ');
                }
                else 
                ...
                // CHANGE MH: Support whitespaces
                var trimhandle = (handle.replace(/\u00A0/g,"łłł").trim()).replace(/łłł/g,"\u00A0");
                rhs = trimhandle.split(' ');
                for (i=0; her = her || rhs[i] === 'error',i<rhs.length; i++) if (!symbols_[rhs[i]]) {
                    addSymbol(rhs[i]);
                }
                r = new Production(symbol, rhs, productions.length+1);
