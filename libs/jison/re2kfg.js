...
switch(arguments[4]) {
case 1:var r = newRuleName(); 
Rules.unshift({"name":"N0","rhs":$$[$0-2+1-1]});
...
$$[$0-2+2-1] = $$[$0-2+2-1].replace(/[{}]/g,"");
var parts = $$[$0-2+2-1].split(",");
var s = "";
for(var x = 0; x < parseInt(parts[0]); x++)
  s += $$[$0-2+1-1] + " ";
Rules.push({"name":r,"rhs":s == "" ? "EPSILON":s});
for(var x = parseInt(parts[0]); x < parseInt(parts[1]); x++){
  s = s + $$[$0-2+1-1] + " ";  
  Rules.push({"name":r,"rhs":s});
}
