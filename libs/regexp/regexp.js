...
var RegexperClasses = require('./lib/grammerClasses');
var RegexperParser = require('./lib/regexpGrammer').Parser;
...
window.renderRegExp = render;

},{"./lib/grammerClasses":2,"./lib/regexpGrammer":14,"./lib/renderer":15}],2:[function(require,module,exports){
...
function AnyCharacter(paper, structure) {
  TextBox.call(this, paper, window.renderRegExpTranslations('ANYCHARACTER'),
...
},{"./text-box":13}],4:[function(require,module,exports){
module.exports = Base;
...
function Charset(paper, structure) {
    ...
    this._label = paper.text(0, 0, structure.inverted ? window.renderRegExpTranslations('NONEOF') : window.renderRegExpTranslations('ONEOF'));
...
},{"../renderer":15,"./base":4}],6:[function(require,module,exports){
var TextBox = require('./text-box');
...
function Escaped(paper, structure) {
    ...
    if (typeof structure.content.type !== 'undefined') {
        ...
        case 'control':
            ...
            break;
        case 'back_reference':
            label = window.renderRegExpTranslations('REFERENCEGROUP',{'GROUP':structure.content.code});
            break;
        case 'octal':
...
},{"./text-box":13}],7:[function(require,module,exports){
var TextBox = require('./text-box');
...
},{"./text-box":13}],8:[function(require,module,exports){
var Base = require('./base');
var TextBox = require('./text-box');
var renderer = require('../renderer');
...
function Match(paper, structure) {
  ...
  renderer.render_contents(paper, structure.content, function(contents) {
    ...
    if (structure.start) {
      self._contents.unshift(new TextBox(paper, window.renderRegExpTranslations('STARTOFLINE'),
        base_anchor_text_attrs, base_anchor_box_attrs));
    }

    if (structure.end) {
      self._contents.push(new TextBox(paper, window.renderRegExpTranslations('ENDOFLINE'),
        base_anchor_text_attrs, base_anchor_box_attrs));
    }
...
Match.prototype._draw_connection = function(a, b) {
  ...
  })).attr(base_connector_arrow_attrs).toBack();
};

Match.prototype.get_connection_offset = function() {
  return this._offset;
};
},{"../renderer":15,"./base":4,"./text-box":13,"raphael":17}],9:[function(require,module,exports){
var Base = require('./base');
var renderer = require('../renderer');
...
},{"../renderer":15,"./base":4}],10:[function(require,module,exports){
var Base = require('./base');
var renderer = require('../renderer');
var Raphael = require('raphael');
...
var item_spacing = 10,
  ...
  base_connector_arrow2_attrs = {
    'arrow-start': 'block-midium-midium',
    'stroke-width': 2
  };
...
},{"../renderer":15,"./base":4,"raphael":17}],11:[function(require,module,exports){
var Base = require('./base');
var Raphael = require('raphael');
var renderer = require('../renderer');
        ...
        if (typeof repeat_count.start !== 'undefined' || typeof repeat_count.stop !== 'undefined') {
            ...
            if (this._include_loop) {
                if (typeof stop === 'undefined') {
                    loop_desc = start +window.renderRegExpTranslations('TIMES');
                } else if (start > 0) {
                    loop_desc = start + '...' + stop;
                } else {
                    loop_desc = window.renderRegExpTranslations('UPTOTIMES',{'TIMES':stop});
                }
            }
        } else {
            this._include_skip = false;
...
Repetition.prototype._draw_skip_line = function() {
    ...
    path_str = 'M{start.x},{start.y}q{radius},0 {radius},-{radius}V{top}q0,-{radius} {radius},-{radius}H{end}';
    ...
    this._paper.path(Raphael.fullfill(path_str, {
        ...
        top: expression_box.y,
        bottom: box.y + offset - curve_radius,
        end: box.x2 - Math.round((box.x2 - box.x  + curve_radius)/2)
    })).attr(base_connector_arrow_attrs).toBack();
...
Repetition.prototype._draw_loop_line = function() {
    ...
    path_str = 'M{start.x},{start.y}q{radius},0 {radius},{radius}V{bottom}';
    ...
    this._paper.path(Raphael.fullfill(path_str, {
        ...
        radius: curve_radius,
        radius2: curve_radius/2,
        top: box.y + offset + curve_radius,
        bottom: expression_box.y2,
        end: expression_box.x
    })).attr(base_connector_arrow_attrs).toBack();

    path_str = 'M{start.x},{start.y}q{radius},0 {radius},{radius}V{bottom}q0,{radius} -{radius},{radius}H{end}q-{radius},0 -{radius},-{radius}V{top}';
...
},{"../renderer":15,"./base":4,"raphael":17}],12:[function(require,module,exports){
var Base = require('./base');
...
function Subexp(paper, structure) {
    ...
    case 'capture':
        label = window.renderRegExpTranslations("GROUP",{"GROUP":structure.group});
        break;
...
},{"./base":4,"./regexp":10,"raphael":17}],13:[function(require,module,exports){
var Base = require('./base');
...
},{"./base":4}],14:[function(require,module,exports){
(function() {
...
var tags = {
    any_character: require('./nodes/any-character'),
    charset: require('./nodes/charset'),
    escaped: require('./nodes/escaped'),
    literal: require('./nodes/literal'),
    match: require('./nodes/match'),
    range: require('./nodes/range'),
    regexp: require('./nodes/regexp'),
    repetition: require('./nodes/repetition'),
    subexp: require('./nodes/subexp')
};

var base_path_attrs = {
        'stroke-width': 2
    },
    base_path_arrow_attrs = {
        'arrow-end': 'block-midium-midium',
        'stroke-width': 2
    },
    base_path_arrow2_attrs = {
        'arrow-start': 'block-midium-midium',
        'stroke-width': 2
    },
    base_anchor_attrs = {
    ...
    Raphael(paper_container, 2, 2, function () {
        ...
        try {
            render(paper, data.structure, function(expression) {
                ...
                expression.stack();
                expression.position(40, 10);

                box = expression.get_box();
                offset = expression.get_connection_offset();

                draw_anchor(paper, 20, box.y + offset, box.x);
                draw_anchorClear(paper, box.x2 + 20, box.y + offset, box.x2);

                paper.setSize(box.width + 80, box.height + 20);
...
exports.draw_anchorClear = draw_anchorClear;
function draw_anchorClear(paper, x, y, connection) {
    paper.path(Raphael.fullfill('M{start.x},{start.y}H{end}', {
        start: {
            x: x-5,
            y: y
    ...
    })).attr(base_path_arrow2_attrs);

    paper.circle(x, y, 5).attr({
        fill: '#fff',
        'stroke-width': 2
    });
}
...
},{"./nodes/any-character":3,"./nodes/charset":5,"./nodes/escaped":6,"./nodes/literal":7,"./nodes/match":8,"./nodes/range":9,"./nodes/regexp":10,"./nodes/repetition":11,"./nodes/subexp":12,"raphael":17}],16:[function(require,module,exports){
...
(function(){
    ...
    addArrow = function (o, value, isEnd) {
        if (o.type == "path") {
            ...
            if (type != "none") {
                ...
                if (!p.defs.querySelector('#'+pathId)) {
                ...
                if (!marker) {
                    ...
                    use = $($("use"), {
                        "xlink:href": top.location.href+"#" + pathId,
                ...
                attr = {};
                attr["marker-" + se] = "url("+top.location.href+"#" + markerId + ")";
                if (to || from) {
                ...
                switch (att) {
                    ...
                    case "clip-rect":
                        ...
                        if (rect.length == 4) {
                            ...
                            $(node, {"clip-path": "url("+top.location.href+"#" + el.id + ")"});
                            o.clip = rc;
                        }
                        if (!value) {
    ...
    elproto.blur = function (size) {
        ...
        if (+size !== 0) {
            ...
            t._blur = fltr;
            $(t.node, {filter: "url("+top.location.href+"#" + fltr.id + ")"});
        } else {
            if (t._blur) {
