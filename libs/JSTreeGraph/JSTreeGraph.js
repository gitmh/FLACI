...
function DrawLines(node, container) {
    if ((!node.Collapsed) && node.Nodes && node.Nodes.length > 0) { // Has children and Is Expanded
        for (var j = 0; j < node.Nodes.length; j++) {
            if(node.Nodes[j].Class.indexOf("invisible") != -1) continue;
...
function DrawNode(node, container, options) {
    ...
    if (node.Content)
        nodeDiv.innerHTML = "<div class='NodeContent'><span>" + node.Content + "</span></div>";
...
function PerformLayoutH(node) {
    ...
    node.Top = (nodeMarginTop * (node.Level + 0)) + (nodeHeight * (node.Level + 1));
    // Size is constant
    node.Height = nodeHeight;
    node.Width = nodeWidth;
